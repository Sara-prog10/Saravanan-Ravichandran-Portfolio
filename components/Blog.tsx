...
const BlogPostPreview: React.FC<{ post: PostType }> = ({ post }) => (
    <a href="#" className="group block p-6 rounded-lg bg-surface dark:bg-dark-surface shadow-md hover:shadow-xl transition-shadow duration-300">
        ...
        <p className="mt-3 text-base text-text-muted dark:text-dark-text-muted">{post.excerpt}</p>
        <div className="mt-4 font-semibold text-primary dark:text-dark-primary inline-flex items-center">
            Read more <ArrowRightIcon className="ml-1 w-4 h-4 transition-transform duration-300 group-hover:translate-x-1" />
        </div>
    </a>
);

export const Blog = forwardRef<HTMLElement, BlogProps>(({ posts }, ref) => {
  return (
